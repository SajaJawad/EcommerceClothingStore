        ...
        {/*---------Product List---------- */}
        {
          list.map((item, index) => (
            <div className='grid grid-cols-[1fr_3fr_1fr_1fr_1fr] items-center gap-2 py-1 px-2 border text-sm' key={index}>
              <img className='w-12 h-12 object-cover rounded' src={Array.isArray(item.image) ? item.image[0] : item.image} onError={(e) => { e.target.src = 'https://via.placeholder.com/100'; }} alt="" />
              <p>{item.name}</p>
              <p>{item.category}</p>
